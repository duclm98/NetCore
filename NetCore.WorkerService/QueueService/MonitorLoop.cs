namespace NetCore.WorkerService.QueueService
{
    public class MonitorLoop
    {
        ...
        private async ValueTask BuildWorkItemAsync(CancellationToken token)
        {
            ...
            while (!token.IsCancellationRequested && delayLoop < totalLoop)
            {
                try
                {
                ...
                _logger.LogInformation("Queued work item {Guid} is running. {DelayLoop}/{TotalLoop}", guid, delayLoop, totalLoop);
            }

            string format = delayLoop switch
            {
                totalLoop => "Queued Background Task {Guid} is complete.",
